...
export default function Settings() {
  ...
  const [brandForm, setBrandForm] = useState<{ name: string; primaryColor: string; logoUrl: string } | null>(null);
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tenant-me'] }); setBrandForm(null); },
  });

  if (isLoading) return <Layout title="Settings"><div className="spinner" /></Layout>;
      ...
      <div style={{ maxWidth: 720 }}>
        ...
        {tab === 'branding' && (
          <div className="card">
            ...
            <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 24 }}>
              ...
              <div>
                ...
                <div style={{ color: 'var(--text-muted)', fontSize: 12 }}>
                  {profile?.stats.deviceCount} devices · {profile?.stats.userCount} users · {profile?.tenant.plan} plan
                </div>
              </div>
            </div>
            ...
            <div className="form-group">
              ...
              <input value={form.name} onChange={e => setBrandForm(f => ({ ...(f ?? form), name: e.target.value }))} />
            </div>
            <div className="form-row">
              <div className="form-group">
                <label className="form-label">Brand colour</label>
                <input type="color" value={form.primaryColor}
                  onChange={e => setBrandForm(f => ({ ...(f ?? form), primaryColor: e.target.value }))}
                  style={{ height: 40, padding: 4 }} />
              </div>
              <div className="form-group">
                <label className="form-label">Logo URL</label>
                <input value={form.logoUrl}
                  onChange={e => setBrandForm(f => ({ ...(f ?? form), logoUrl: e.target.value }))}
        ...
        {tab === 'audit' && (
          <div className="table-wrapper">
            <table>
              <thead>
                <tr><th>Action</th><th>Actor</th><th>Resource</th><th>Time</th></tr>
              </thead>
              <tbody>
                {(auditLogs?.content ?? []).map((log: any) => (
                  <tr key={log.id}>
                    <td><code style={{ fontSize: 12 }}>{log.action}</code></td>
                    <td style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{log.actorEmail ?? '—'}</td>
...
function TeamTab() {
  const { data: users = [] } = useQuery({
    queryKey: ['users'],
    queryFn: () => import('../lib/api').then(m => m.tenant.users()).then(r => r.data),
  });
