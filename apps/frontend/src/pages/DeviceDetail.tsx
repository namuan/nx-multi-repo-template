...
import { ArrowLeft, MapPin, Gauge, Activity } from 'lucide-react';
...
export default function DeviceDetail() {
  ...
  if (!device)
    return (
      <Layout title="Device">
        <div className="empty-state">Device not found</div>
      </Layout>
    );
  ...
  return (
    <Layout title={device.name}>
      ...
      >
        {[
          {
            icon: <Gauge size={17} color="var(--brand)" />,
            label: 'Speed',
            value: `${device.lastSpeed?.toFixed(1) ?? 0} km/h`,
            bg: 'rgba(59,130,246,0.1)',
            accent: 'var(--brand)',
          },
          {
            icon: <Activity size={17} color="var(--success)" />,
            label: 'Status',
            value: device.status,
          ...
          {
            icon: <MapPin size={17} color="var(--warning)" />,
            label: 'Location',
            value: device.lastLat
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 16 }}>
        <div className="card">
          <div style={{ fontWeight: 600, marginBottom: 16 }}>
            Telemetry History (last 100 events)
          </div>
          {loadingTelemetry ? (
            <div className="spinner" />
          ) : chartData.length === 0 ? (
        ...
        <div className="card">
          ...
          {[
            ...
            {
              label: 'Status',
              value: <span className={`badge badge-${device.status}`}>{device.status}</span>,
            },
          ].map((row) => (
            <div
              key={row.label}
