      ...
      <div className="modal">
        ...
        <div className="form-group">
          ...
          <input
            value={form.name}
            onChange={set('name')}
            placeholder="Speed limit 80 km/h"
            required
          />
        </div>
        ...
        <>
          <div className="page-header">
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                className={`btn ${filter === 'unacked' ? 'btn-primary' : 'btn-secondary'}`}
              ...
              <tbody>
                {rules.map((r: AlertRule) => (
                  <tr key={r.id}>
                    ...
                    <td style={{ color: 'var(--text-muted)' }}>{r.type.replaceAll('_', ' ')}</td>
